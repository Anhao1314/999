...
function renderInspector(){
  ...
  $('inspector-content').innerHTML=`<div class="employee-profile">${avatar(e)}<div><h2>${e.name}</h2><span class="employee-state"><i class="status-dot ${e.state==='Available'?'blue-dot':''}"></i>${e.state}</span><p class="employee-role">${e.role}</p><p class="employee-quote">“${e.quote}”</p></div></div><section class="inspector-section"><h3>当前工作</h3>${e.work?`<button class="inspector-work" data-work="${adapter.works().find(w=>w.title===e.work)?.id??'102'}"><span>${icon('alert')}</span><span><strong>${e.work}</strong><small>${e.state==='Reviewing'?'评审中':e.state==='Working'?'执行中':'待开始'}</small></span><span>${icon('chevron-right')}</span></button>`:'<p class="modal-hint">暂未分配工作，等待新的想法。</p>'}</section><section class="inspector-section"><h3>能力标签</h3><div class="tags">${e.skills.map(s=>`<span>${s}</span>`).join('')}</div></section><section class="inspector-section"><h3>执行环境</h3><div class="environment"><span class="terminal-icon">${icon('terminal')}</span>Codex Local</div></section><section class="inspector-section artifact-list"><h3>最近交付物</h3>${[['Export API','api'],['User Auth Fix','patch'],['Docs Improvement','research']].map(([name,id])=>`<button class="side-artifact" data-artifact="${id}"><span class="file-symbol">${icon('file')}</span><span>${name}</span>${badge(id==='research'?'REVIEW':'PASS')}</button>`).join('')}<button class="full-profile" data-action="employee-profile">查看完整档案 ${icon('arrow-right')}</button></section><section class="quick-actions"><h3>快捷操作</h3><div class="quick-action-grid"><button data-action="message"><span>${icon('message')}</span>发送消息</button><button data-action="employee-work"><span>${icon('work')}</span>查看工作</button><button data-action="employee-profile"><span>${icon('more')}</span>更多</button></div></section><p class="inspector-footnote">示例员工 · 执行环境尚未连接</p>`;
}
...
function navigate(page,{updateHash=true}={}){
  ...
  if(page==='employees')html=`<div class="section-head"><h2>你的团队</h2><span>8 位成员 · 示例员工</span></div><div class="employee-grid">${employees.map(e=>`<button class="employee-tile" data-employee="${e.id}">${avatar(e)}<strong>${e.name}</strong><small>${e.role}</small><span class="employee-status"><i class="status-dot ${e.state==='Available'?'blue-dot':''}"></i>${e.state}</span></button>`).join('')}</div>`;
  ...
  if(page==='hiring')html=`<div class="empty-panel"><span class="color-icon blue">${icon('users')}</span><h2>下一位伙伴，会带来什么？</h2><p>从公司的需要出发，明确岗位、能力和权限，再通过试用工作确认入职。招聘流程将在后续接入。</p><button class="primary-button" data-action="hiring-preview">看看岗位需求示例</button></div>`;
...
function showSearch(){
  ...
  const render=()=>{const query=$('global-search').value.trim().toLowerCase();const foundPeople=employees.filter(e=>`${e.name} ${e.role}`.toLowerCase().includes(query));const foundWorks=adapter.works().filter(w=>w.title.toLowerCase().includes(query));$('search-results').innerHTML=[...foundWorks.map(w=>`<button data-work="${escape(w.id)}"><span class="file-symbol">${icon('work')}</span><span>${escape(w.title)}<small>工作 · ${w.status}</small></span></button>`),...foundPeople.map(e=>`<button data-employee="${e.id}" data-close-after>${avatar(e)}<span>${e.name}<small>${e.role}</small></span></button>`)].join('')||'<p>没有找到匹配的工作或员工。</p>';};
...
function showProfile(){const e=employee();openModal(`${e.name} 的员工档案`,`<div class="modal-employee">${avatar(e)}<div><h3>${e.name}</h3><p>${e.role} · ${e.state}</p></div></div><div class="detail-box"><h3>工作理念</h3><p>“${e.quote}”</p></div><div class="detail-box"><h3>岗位能力</h3><div class="tags">${e.skills.map(s=>`<span>${s}</span>`).join('')}</div></div><p class="modal-hint">示例身份。模型、工具授权、配置保存和真实执行将在后续接入。</p>`);}
...
$('team-avatars').innerHTML=employees.slice(0,4).map(e=>`<button class="avatar-button" data-employee="${e.id}" aria-label="查看 ${e.name}">${avatar(e)}</button>`).join('')+'<button class="more-people" data-nav="employees" aria-label="查看全部八位员工">+4</button>';
