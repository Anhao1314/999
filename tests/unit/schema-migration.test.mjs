...
test("a real v6 store adds the Founder binding table without rebinding historical Work", () => {
  const dir = tempStoreDir();
  try {
    const seed = openKernel({ dir });
    const company = seed.createCompany({ name: "Pre-product company" });
    const work = seed.createWork({ companyId: company.id, title: "Legacy Work", intent: "Preserve this Work" });
    const before = seed.workProjection(work.id);
    seed.close();

    const db = new DatabaseSync(join(dir, STORE_FILE_NAME));
    db.exec("DROP TABLE founder_work_execution_bindings");
